...
def focal_loss(
    ...
    target_one_hot: torch.Tensor = F.one_hot(target, num_classes=input.shape[1]).type_as(input)
...
class FocalLoss(nn.Module):
    ...
    """
    @configurable
    def __init__(self, alpha: float = 1.0, gamma: float = 0.0, reduction: str = 'mean', eps: float = 1e-8) -> None:
        super().__init__()
