...
class RepVGGEncoder(RepVGG, EncoderMixin):
    def __init__(self, num_blocks, width_multiplier, deploy, use_se, depth=5):
        super().__init__(num_blocks=num_blocks, width_multiplier=width_multiplier, deploy=deploy, use_se=use_se)
        self._depth = depth
        self._out_channels = (3, min(64, int(64 * width_multiplier[0])),
...
repvgg_encoders = {
    'repvgg_s0': {
        ...
        'params': {
            'num_blocks': [2, 4, 14, 1],
            'width_multiplier': [0.1, 0.1, 0.1, 0.25],
            'deploy': False,
        }
    },
    'repvgg_s1': {
