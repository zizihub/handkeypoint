...
def get_params_rsc(img, scale, ratio):
    img = np.array(img[0])
...
class MotionAugmentation:
    ...
    def __call__(self, fgrs, phas, bgrs):
        ...
        # Resize
        params = get_params_rsc(fgrs, scale=(1, 1), ratio=self.aspect_ratio_range)
        fgrs = albu_F.random_crop(np.array(fgrs[0]), **params)
        fgrs = [Image.fromarray(albu_F.resize(fgrs, *self.size))]
        phas = albu_F.random_crop(np.array(phas[0]), **params)
        phas = [Image.fromarray(albu_F.resize(phas, *self.size))]
        params = get_params_rsc(bgrs, scale=(1, 1), ratio=self.aspect_ratio_range)
        bgrs = albu_F.random_crop(np.array(bgrs[0]), **params)
        bgrs = [Image.fromarray(albu_F.resize(bgrs, *self.size))]

        # ZeroPadding
        fgrs, phas, bgrs = self._zero_padding(fgrs, phas, bgrs)
        ...
        # fgrs = F.resized_crop(fgrs, *params, self.size, interpolation=Image.BILINEAR)
        # phas = F.resized_crop(phas, *params, self.size, interpolation=Image.BILINEAR)
        # params = transforms.RandomResizedCrop.get_params(bgrs, scale=(1, 1), ratio=self.aspect_ratio_range)
        # bgrs = F.resized_crop(bgrs, *params, self.size, interpolation=Image.BILINEAR)
        ...
        if random.random() < self.prob_color_jitter:
            fgrs, bgrs = self._motion_color_jitter(fgrs, bgrs)
    ...
    def _zero_padding(self, *imgs):
        out = []
        for img in imgs:
            # ! static image only
            img = img[0]
            img_edge = max(img.size)
            out.append([ImageOps.pad(img, (img_edge, img_edge))])
        return out
    ...
    def _static_affine(self, *imgs, scale_ranges):
        ...
        imgs = [[F.affine(t, *params, Image.BILINEAR) for t in img] for img in imgs]
