...
class ImgWtLossSoftNLL(nn.Module):
    ...
    def custom_nll(self, inputs, target, class_weights, border_weights, mask):
        ...
        """
        loss_matrix = (-1 / border_weights * (target.float() * class_weights.unsqueeze(0).unsqueeze(2).unsqueeze(3)
...
class JointEdgeSegLoss(nn.Module):
    @configurable
    def __init__(self, classes, ignore_index=255, edge_weight=25, seg_weight=1, seg_body_weight=0.1, att_weight=1):
        super(JointEdgeSegLoss, self).__init__()
        self.num_classes = classes
        self.seg_loss = DC_and_Focal_loss(soft_dice_kwargs={'batch_dice': False, 'do_bg': True, 'smooth': 1., 'square': False},
                                          focal_kwargs={'alpha': 0.25, 'gamma': 2, 'balance_index': 0, 'smooth': 1e-5}).cuda()
        # self.seg_body_loss = ImgWtLossSoftNLL(classes=classes,
        #                                       ignore_index=ignore_index,
        #                                       upper_bound=1.0, ohem=False).cuda()
        self.edge_ohem_loss = OhemCrossEntropy2dTensor(ignore_index=ignore_index).cuda()
    ...
    def forward(self, outs, targets, **kwargs):
        seg_in, edge_in = outs['masks'], outs['seg_edge_out']
        segmask, edgemask = targets, kwargs['edge_map']
        losses = {}
        losses['seg_loss'] = self.seg_weight * self.seg_loss(seg_in, segmask)
        # if self.seg_body_weight:
        #     seg_body = self.seg_body_weight * self.seg_body_loss(seg_body_in, seg_bord_mask)
        #     # * skip nan loss for bp
        #     if torch.isnan(seg_body):
        #         pass
        #     else:
        #         losses['seg_body'] = seg_body
        if edge_in.shape != edgemask.shape:
            edge_in = F.interpolate(edge_in, edgemask.shape[-2:], mode='bilinear', align_corners=True)
        losses['edge_loss'] = self.edge_weight * self.bce2d(edge_in, edgemask)
