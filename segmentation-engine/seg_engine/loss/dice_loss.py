...
import torch
from .boundary_loss import softmax_helper, sum_tensor
from .ND_Crossentropy import CrossentropyND, WeightedCrossEntropyLoss, TopKLoss
...
class GDiceLoss(nn.Module):
    ...
    def forward(self, net_output, gt):
        ...
        intersection: torch.Tensor = w * \
            einsum("bcxyz, bcxyz->bc", softmax_output, y_onehot)
        union: torch.Tensor = w * \
            (einsum("bcxyz->bc", softmax_output) + einsum("bcxyz->bc", y_onehot))
        divided: torch.Tensor = 1 - 2 * \
...
class GDiceLossV2(nn.Module):
    ...
    def forward(self, net_output, gt):
        ...
        class_weights = Variable(
            1. / (target_sum * target_sum).clamp(min=self.smooth), requires_grad=False)
...
class SSLoss(nn.Module):
    ...
    def forward(self, net_output, gt, loss_mask=None):
        ...
        specificity_part = sum_tensor(
            squared_error*y_onehot, axes)/(sum_tensor(y_onehot, axes)+self.smooth)
        sensitivity_part = sum_tensor(
            squared_error*bg_onehot, axes)/(sum_tensor(bg_onehot, axes)+self.smooth)
...
class SoftDiceLossV2(nn.Module):
    ...
    def forward(self, x, y):
        ...
        dc = 2.0 * (intersection + self.smooth) / \
            (ground_o + pred_o + self.smooth)
...
class AsymLoss(nn.Module):
    ...
    def forward(self, x, y, loss_mask=None):
        ...
        asym = (tp + self.smooth) / (tp + weight *
                                     fn + (1-weight)*fp + self.smooth)
...
class DC_and_Focal_loss(nn.Module):
    ...
    def forward(self, out, target, **kwargs):
        ...
        if isinstance(net_output, tuple):
            dc_loss, focal_loss = 0.0, 0.0
            for net_out in net_output:
                dc_loss += self.dc(net_out, target)
                focal_loss += self.focal(net_out, target)
        else:
            dc_loss = self.dc(net_output, target)
