...
def compute_edts_forhdloss(segmentation):
    ...
    for i in range(segmentation.shape[0]):
        ...
        negmask = ~posmask
        res[i] = distance_transform_edt(
            posmask) + distance_transform_edt(negmask)
    return res
...
class DistBinaryDiceLoss(nn.Module):
    ...
    def forward(self, net_output, gt):
        ...
        dc = (2 * tp + self.smooth) / (torch.sum(net_output[:, 1, ...], (1, 2, 3)) + torch.sum(
            y_onehot[:, 1, ...], (1, 2, 3)) + self.smooth)
