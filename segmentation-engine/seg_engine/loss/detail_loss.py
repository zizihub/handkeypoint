...
def get_boundary(gtmasks):
    ...
    boundary_targets = F.conv2d(gtmasks, laplacian_kernel, padding=1)
...
class DetailAggregateLoss(nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        self.dc_fc = DC_and_Focal_loss()

    def forward(self, outs, targets, **kwargs):
        feat_out_sp8 = outs['boundary_8']
        feat_out_sp4 = outs['boundary_4']
        feat_out_sp2 = outs['boundary_2']
        loss = dict()
        loss['dice_focal'] = self.dc_fc(outs, targets)
        loss['boundary_bce_loss'], loss['boundary_dice_loss'] = 0, 0
        bce_loss, dice_loss = self.forward_detail_path(feat_out_sp8, targets)
        loss['boundary_bce_loss'] += bce_loss
        loss['boundary_dice_loss'] += dice_loss
        bce_loss, dice_loss = self.forward_detail_path(feat_out_sp4, targets)
        loss['boundary_bce_loss'] += bce_loss
        loss['boundary_dice_loss'] += dice_loss
        bce_loss, dice_loss = self.forward_detail_path(feat_out_sp2, targets)
        loss['boundary_bce_loss'] += bce_loss
        loss['boundary_dice_loss'] += dice_loss
        return loss

    def forward_detail_path(self, boundary_logits, gtmasks):
        # boundary_logits = boundary_logits
