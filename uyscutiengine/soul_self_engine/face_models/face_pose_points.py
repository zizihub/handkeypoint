...
from utils.utils import azimuth_angle

class FaceDirection(object):
    def __init__(self):
        super(FaceDirection, self).__init__()
        model_path = "./models/face_pose_points.mnn"
        self.input_size = 64
    ...
    def preprocess(self, input_image):
        image = pad_and_resize(input_image, [self.input_size, self.input_size], padding= True, norm_flag = 0)
    ...
    def cal_direction(self, point_u, point_d):
        aza = azimuth_angle(point_u[0], point_u[1], point_d[0], point_d[1])
    ...
    def get_face_direction(self, input_image):
        ...
        point_l = landmarks[2]
        point_r = landmarks[4]
        point_u = (point_l + point_r) / 2 
        direction = self.cal_direction(point_u, point_d)
...
def parse_args():
    ...
    parser.add_argument('--input_dir', default="./test_input/mouth_imgs", type=str, help="input dir for images")
    parser.add_argument('--save_dir', default="./test_output/mouth_landmark", type=str, help="output_dir to save datas")
...
def main():
    ...
    for file in os.listdir(args.input_dir):
        ...
        draw_img = img.copy()
        landmarks = mouth_landmark.get_landmark(img)
        # print(face_boxes)
